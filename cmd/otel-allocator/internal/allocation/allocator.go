...
func (a *allocator) SetTargets(targets []*target.Item) {
	begin := time.Now()
	defer a.timeToAssign.Record(context.Background(), time.Since(begin).Seconds(), metric.WithAttributes(attribute.String("method", "SetTargets"), attribute.String("strategy", a.strategy.GetName())))
...
func (a *allocator) SetCollectors(collectors map[string]*Collector) {
	begin := time.Now()
	defer a.timeToAssign.Record(context.Background(), time.Since(begin).Seconds(), metric.WithAttributes(attribute.String("strategy", a.strategy.GetName()), attribute.String("method", "SetCollectors")))
