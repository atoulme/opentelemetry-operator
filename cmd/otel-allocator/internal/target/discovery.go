...
func (m *Discoverer) Reload() {
	...
	begin := time.Now()
	defer m.processTargetsDuration.Record(context.Background(), time.Since(begin).Seconds())
...
func (m *Discoverer) processTargetGroups(jobName string, groups []*targetgroup.Group, intoTargets []*Item) {
	...
	begin := time.Now()
	defer m.processTargetGroupsDuration.Record(context.Background(), time.Since(begin).Seconds(), metric.WithAttributes(attribute.String("job.name", jobName)))
	var count float64 = 0
	index := 0
